...
import 'package:task_app/utils/db_keys.dart';
...
class TaskDatasource {
  ...
  Future<Database> _initDB() async {
    ...
    return openDatabase(path, version: 15, onCreate: _onCreate);
  }
  ...
  Future<void> _onCreate(Database db, int version) async {
    await db.execute('''
      CREATE TABLE ${DBKeys.dbTable} (
        ...
        ${DBKeys.isCompleteColumn} INTEGER,
...
''');
  ...
  Future<int> addTask(Task task) async {
    ...
    return db.transaction((txn) async {
      return txn.insert(DBKeys.dbTable, task.toJson(),
  ...
  Future<int> deleteTask(Task task) async {
    final db = await database;
    return db.transaction((txn) async {
      return txn.delete(DBKeys.dbTable, where: 'id = ?', whereArgs: [task.id]);
    });
  }
